...
def verify_reference_integrity(db, table_name, key_name, referenced_table):
    ...
    for id, row in db[table_name].items():
        ...
        nullish_references = [None, 0, -1]  # interpret these as intentional missing values
        if (reference_id not in nullish_references) and (reference_id not in db[referenced_table]):
